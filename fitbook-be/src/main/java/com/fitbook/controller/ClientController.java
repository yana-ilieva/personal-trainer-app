...
public class ClientController {
    ...
    public List<ProgressDto> getProgress(@PathVariable("id") Long id,
                                         ...
                                         Authentication a) {
        validator.checkClientAccessRights(id, a);
        return clientService.getProgress(id, page, size);
    }

    @GetMapping("/user/{user_id}")
    @Secured("ROLE_CLIENT")
    public ClientDto findClientByUserId(@PathVariable("user_id") Long id, Authentication a) {
        validator.checkClientAccessRightsByUser(id, a);
        return clientService.findClientByUserId(id);
    }

    @PutMapping("/{id}")
    @Secured("ROLE_CLIENT")
    public ClientDto update(@PathVariable("id") Long id, @RequestBody ClientDto clientDto, Authentication a) {
        validator.checkClientAccessRights(id, a);
